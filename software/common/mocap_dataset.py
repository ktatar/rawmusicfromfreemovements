    ...
    def compute_positions(self):
        ...
        for subject in self._data.values():
            for action in subject.values():
                ...
                rotations = np.expand_dims(np.copy(action['rotations']).astype(np.float32), axis=0)
                trajectory = np.expand_dims(np.copy(action['trajectory']).astype(np.float32), axis=0)
                
                action['positions_world'] = tf.squeeze(self._skeleton.forward_kinematics( tf.convert_to_tensor(rotations), tf.convert_to_tensor(trajectory)), axis=0).numpy()
                
                """
                # Absolute translations across the XY plane are removed here
                trajectory[:, :, [0, 0, 2]] = 0
                action['positions_local'] = tf.squeeze(self._skeleton.forward_kinematics( tf.convert_to_tensor(rotations), tf.convert_to_tensor(trajectory)), axis=0).numpy()        
                """
                
                # set root position to zero for calculating local joint positions
                zero_trajectory = np.zeros(shape=trajectory.shape, dtype=np.float32)
                action['positions_local'] = tf.squeeze(self._skeleton.forward_kinematics( tf.convert_to_tensor(rotations), tf.convert_to_tensor(zero_trajectory)), axis=0).numpy()        
