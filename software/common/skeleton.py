...
class Skeleton:
    def __init__(self, offsets, parents):
        ...
        self._offsets = tf.convert_to_tensor(offsets, dtype=tf.float32)
    ...
    def remove_joints(self, joints_to_remove, dataset):
        ...
        self._offsets = tf.gather(self._offsets, valid_joints)
    ...
    def forward_kinematics(self, rotations, root_positions):
        ...
        expanded_offsets = tf.broadcast_to(self._offsets, [rotations.shape[0], rotations.shape[1],
        ...
        return tf.keras.backend.permute_dimensions(tf.stack(positions_world, axis=3), pattern=(0, 1, 3, 2))
